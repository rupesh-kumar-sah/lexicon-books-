  ...
  return (
    ...
    >
      ...
      <Link to={`/book/${book.id}`} className="block relative mb-4">
        <div className="aspect-[3/4] bg-slate-200 rounded-lg overflow-hidden flex items-center justify-center text-slate-400 font-serif italic text-lg p-4 text-center">
          <img 
            src={book.coverImage} 
            alt={book.title} 
          ...
          />
          {!book.coverImage && <span className="relative z-10">{book.title}</span>}
          ...
          {book.stock === 0 && (
            <div className="absolute inset-0 bg-white/40 backdrop-blur-[1px] flex items-center justify-center z-20">
              <span className="bg-slate-800 text-white text-[10px] uppercase tracking-tighter px-2 py-1 rounded">Stock Alert</span>
            </div>
          )}
        </div>
      </Link>
      ...
      <div className="flex flex-col flex-1">
        <h4 className="font-bold text-sm mb-1 line-clamp-1 text-slate-900">{book.title}</h4>
        ...
        <div className="mt-auto flex items-center justify-between">
          <div className="flex flex-col">
            ...
            {book.stock <= 5 && book.stock > 0 && (
              <span className="text-[10px] text-orange-500 font-bold uppercase tracking-tight">Only {book.stock} left</span>
            )}
          </div>
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white text-xs font-bold rounded-xl hover:bg-blue-600 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100 shadow-sm"
          >
