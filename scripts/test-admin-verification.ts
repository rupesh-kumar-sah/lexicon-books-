...
try {
  ...
  const missingLocation = await login({ device: 'Samsung F23 Chrome' });
  ...
  const validDevice = await login({ latitude: 27.7172, longitude: 85.324, device: 'Mozilla/5.0 Samsung F23 Chrome' });
  if (validDevice.status !== 200 || !validDevice.body.token || validDevice.body.user?.role !== 'admin') {
    throw new Error(`Valid Samsung F23 request did not authenticate: ${validDevice.status}`);
  }
  ...
  console.log('ADMIN SAMSUNG F23 AND GEOLOCATION TEST PASSED');
} finally {
